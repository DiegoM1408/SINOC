...
<head>
    ...
    <style>
        /* Mantener todo el CSS anterior sin cambios */
        :root {
...
<body id="body" class="<?php echo $_SESSION['dark_mode'] ? 'dark-mode' : ''; ?>">
    ...
    <script>
        ...
        function getCachedData(tableId) {
            const cached = localStorage.getItem(`cache_fuera_horarios_${tableId}`);
            ...
            if (now - timestamp > CACHE_DURATION) {
                localStorage.removeItem(`cache_fuera_horarios_${tableId}`);
                return null;
            }
        ...
        function setCachedData(tableId, data) {
            ...
            localStorage.setItem(`cache_fuera_horarios_${tableId}`, JSON.stringify(cacheItem));
        ...
        async function loadTable(tableId) {
            ...
            try {
                const response = await fetch(`load_fuera_horarios.php?table=${tableId}`);
